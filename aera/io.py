...
def store_metadata_oa(
        meta_file, arag_target_rel, arag_target_abs, year_x, model_start_year,
        s_arag_anth, s_total_emission, s_ff_emission, emission_curve):
    meta_file = Path(meta_file)
    timeseries_csv = Path(str(meta_file) + '.timeseries.csv')
    scalar_csv = Path(str(meta_file) + '.scalar.csv')

    timeseries_columns = [
        'total_emission', 'ff_emission', 'arag_anth', 'arag_anth_rel']
    scalar_columns = [
        'arag_target_abs', 'total_emission_budget', 'ff_emission_budget',
        'reb', 'ec_cost', 'ec_reb_diff', 'ec_overshoot_integral', 'ec_slope_t1',
        'ec_slope_change', 'ec_overshoot','ec_curvature', 'ec_target_year_rel',
        'ec_a', 'ec_b', 'ec_c', 'ec_d',
        ]
    ...
    df_scalar.loc[year_x, 'arag_target_abs'] = arag_target_abs
    target_year_abs = emission_curve.target_year_rel + year_x
    df_scalar.loc[year_x, 'total_emission_budget'] = (
        s_total_emission.loc[:target_year_abs].sum())
    df_scalar.loc[year_x, 'ff_emission_budget'] = (
    ...
    for year in s_total_emission.loc[:target_year_abs].index.values:
        df_timeseries.loc[
            (year_x, year), 'total_emission'] = s_total_emission.loc[year]
    for year in s_arag_anth.loc[:target_year_abs].index.values:
        df_timeseries.loc[
            (year_x, year), 'arag_anth'] = s_arag_anth.loc[year]
        df_timeseries.loc[
            (year_x, year), 'arag_anth_rel'] = (
                  s_arag_anth.loc[year] - arag_target_abs + arag_target_rel)
    df_timeseries.to_csv(timeseries_csv)
